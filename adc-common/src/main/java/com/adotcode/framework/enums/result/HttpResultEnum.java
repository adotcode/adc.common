package com.adotcode.framework.enums.result;
 ...
 * @date 2019/07/24
 */
public enum HttpResultEnum implements IEnum<String> {

  /**
   * ok
   */
  OK("OK", "result.code.ok"),

  /**
   * FAILED
   */
  FAILED("FAILED", "result.code.failed"),
   ...
   */
  HttpResultEnum(String code, String reasonPhrase) {
    this.code = code;
   ...
   */
  public static HttpResultEnum of(String code) {
    HttpResultEnum httpResultEnum = resolve(code);
  ...
  /**
   * Resolve the given code to an {@code HttpResultEnum}, if possible.
   *
   * @param code the code (potentially non-standard)
   * @return the corresponding {@code HttpResultEnum}, or {@code null} if not found
   */
  @Nullable
  public static HttpResultEnum resolve(String code) {
    for (HttpResultEnum httpResultEnum : values()) {
      if (httpResultEnum.value().equals(code)) {
        return httpResultEnum;
      }
    }
    return null;
